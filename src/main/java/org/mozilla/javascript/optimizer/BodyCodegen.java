...
public class BodyCodegen {
    ...
    private void generatePrologue() {
        ...
        if (hasVarsInRegs) {
            ...
            List<Object> defaultParams = fnCurrent.fnode.getDefaultParams();
            ...
            for (int i = 0; i != varCount; ++i) {
                ...
                if (i < paramCount) {
                    if (!inDirectCallFunction) {
                        ...
                        Object possible = defaultParams == null || i > (defaultParams.size() - 1) ? null : defaultParams.get(i);
                        if (possible instanceof AstNode) {
                            AstNode possibleNode = (AstNode) possible;
                            if (possibleNode.getType() == Token.THROW) {
                                int label = cfw.acquireLabel();
                                cfw.addALoad(argsLocal);
                                cfw.addPush(i);
                                cfw.add(ByteCode.AALOAD);
                                ...
                                generateExpression(possibleNode, fnCurrent.fnode);
                                cfw.markLabel(label);
                            }
                        }
                        cfw.addAStore(reg);
    ...
    private void generateEpilogue() {
        ...
        if (isGenerator) {
            ...
            if (currentCtorClass) {
                cfw.addALoad(variableObjectLocal);
                addScriptRuntimeInvoke("endClassCtor", SCRIPTABLE, SCRIPTABLE);
            }
    ...
    private void generateExpression(Node node, Node parent) {
        ...
        switch (type) {
            ...
            case Token.THROW:
                // For some reason this is sometimes null, i have absolutely no idea why that is
                // however hopefully this shitty workaround fixes it without further issues right ?
                if (child == null) break;
                generateExpression(child, node);
                if (compilerEnv.isGenerateObserverCount())
                    addInstructionCount();
            ...
            case Token.NAME:
                {
                    if (node.getString().equals("new.target")) {
                        if (scriptOrFn.getType() == Token.FUNCTION) {
                            cfw.addLoadThis();
                            cfw.addALoad(thisObjLocal);
                            addScriptRuntimeInvoke("getNewTarget", OBJECT, OBJECT, OBJECT);
                            break;
                        }
                    }
                    cfw.addALoad(contextLocal);
    ...
    private void visitClass(ClassNode cls) {
        ...
        while (child != null) {
            if (child instanceof ClassMethod) {
                ...
                ClassField classField = (ClassField) child;
                Node defaultValue = classField.getFirstChild();
                Object name = classField.getNameKey();
                ...
                cfw.addALoad(contextLocal);
                cfw.addPush(classField.isPrivate());
        ...
        if (firstArgChild == null) {
            if (childType == Token.NAME) {
                ...
                if (child.getProp(Node.SUPER_PROP) != null) {
                    ...
                    if (child.getProp(Node.SUPER_PROP) == GENERATED_SUPER) {
                        cfw.addALoad(argsLocal);
                    } else {
                        cfw.add(ByteCode.ACONST_NULL);
                    }
...
//            ++argCount;
            ...
            case Token.GETELEM:
                {
                    ...
                    if (type == Token.GETPROP) {
                        ...
                        cfw.addALoad(contextLocal);
                        cfw.addALoad(variableObjectLocal);
                        String propAccessType = isOptionalChainingCall ? "getPropFunctionAndThisOptional" : "getPropFunctionAndThis";
                        cfw.addInvoke(
                                ByteCode.INVOKESTATIC,
                                "org/mozilla/javascript/ScriptRuntime",
                                propAccessType,
            ...
            case Token.NAME:
                {
                    ...
                    cfw.addALoad(contextLocal);
                    cfw.addALoad(variableObjectLocal);
                    String propAccessType = isOptionalChainingCall ? "getNameFunctionAndThisOptional" : "getNameFunctionAndThis";
                    cfw.addInvoke(
                            ByteCode.INVOKESTATIC,
                            "org/mozilla/javascript/ScriptRuntime",
                            propAccessType,
    ...
    private void finishGetPropGeneration(Node node, Node child) {
        ...
        String propName = nameChild.getString();
