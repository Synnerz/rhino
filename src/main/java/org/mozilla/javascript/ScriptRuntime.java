...
public class ScriptRuntime {
    ...
    public static Object callWithTemplateLiteral(
        ...
        NativeArray parts = (NativeArray) cx.newArray(scope, Arrays.copyOfRange(args, 0, boundary));
    ...
    public static Object addClassMethod(Object clazzObj, Object name, Object method, Context cx, boolean instance, int getterSetter, boolean isPrivate) {
        ...
        if (name instanceof String) {
            ...
            if (getterSetter == 0) {
                ...
                clazz.setGetterOrSetter(String.valueOf(nameInt), 0, getterOrSetter, isSetter);
            }
...
//                setFunctionNameIfApplicable(method, (isSetter ? "set " : "get ") + nameSymbol.toSymbolString());
                clazz.setGetterOrSetter(nameSymbol.toString(), 0, getterOrSetter, isSetter);
                clazz.setAttributes(nameSymbol, clazz.getAttributes(nameSymbol) | ScriptableObject.DONTENUM);
            }
        } else {
    ...
    public static Object callSuper(Object[] args, boolean isReturned, NativeFunction clazz, Scriptable thisObj, Scriptable scope, Context cx) {
        ...
        if (!(proto instanceof Callable)) {
            throw Kit.codeBug("Prototype is not callable");
        }

        if (isReturned) {
            if (!(proto instanceof BaseFunction)) {
                throw Kit.codeBug("Prototype is not a BaseFunction");
            }
            return ((BaseFunction) proto).construct(cx, scope, args, thisObj);
        }

        Object result = ((Callable) proto).call(cx, scope, thisObj, args);
        return thisObj;
    }
        ...
        for (; ; ) {
            ...
            if (val instanceof Scriptable) {
                if (val instanceof NativeArray) {
                    ...
                    return NativeJSON.stringify(Context.getContext(), Context.getContext().topCallScope, val, null, 2).toString();
                }
                val = ((Scriptable) val).getDefaultValue(StringClass);
                quoteString = false;
                if ((val instanceof Scriptable) && !isSymbol(val)) {
    ...
    public static void handleExport(String targetName, String scopeName, Scriptable fromRequire, Scriptable scope) {
        ...
        if (ScriptableObject.hasProperty((Scriptable) exports, scopeName)) {
    ...
    public static void handleExport(String targetName, String scopeName, Scriptable scope) {
        ...
        if ("default".equals(scopeName)) {
            if (ScriptableObject.hasProperty((Scriptable) exports, "default")) {
    ...
    public static void handleExport(Object value, Scriptable scope) {
        ...
        ScriptableObject.putConstProperty((Scriptable) exports, "default", value);
    }

    private static Object getExports(Scriptable scope) {
        return ScriptableObject.getProperty((Scriptable) ScriptableObject.getProperty(scope, "module"), "exports");
    }
